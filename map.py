...
class Map:
    ...
    def draw_dynamic_overlay(self, surface, player, before_player=True):
        # Draw overlay elements based on their vertical position relative to the player
                ...
                for tile_key in tile_keys:
                    if tile_key in self.tiles:
                        ...
                        if z_index > 0:  # Only consider overlay elements
                            ...
                            if (before_player and draw_y + tile_height <= player_y + player.desired_height // 2) or \
                               (not before_player and draw_y + tile_height > player_y + player.desired_height // 2):
                                surface.blit(tile, (draw_x, draw_y))
